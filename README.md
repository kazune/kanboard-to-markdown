...
chmod 600 ~/.config/kanboard-md/config.json
```
...
API通信には10秒の接続タイムアウトと60秒の全体タイムアウトを設定します。APIトークンはcurlのコマンドライン引数には渡しません。
...
```bash
./kanboard-md --config ~/.config/kanboard-md/config.json boards
./kanboard-md --config ~/.config/kanboard-md/config.json board <board_id>
./kanboard-md --config ~/.config/kanboard-md/config.json task <task_id>
```
...
```bash
./kanboard-md --config ~/.config/kanboard-md/config.json boards
./kanboard-md --config ~/.config/kanboard-md/config.json boards > boards.md
```
...
```bash
./kanboard-md --config ~/.config/kanboard-md/config.json board 3
```

ファイルへ保存する場合:

```bash
./kanboard-md --config ~/.config/kanboard-md/config.json board 3 > board.md
```
...
```bash
./kanboard-md --config ~/.config/kanboard-md/config.json task 63 > task-63.md
```
